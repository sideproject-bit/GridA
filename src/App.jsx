...
function AppShell() {
  ...
  const { unreadDirect, unreadGroups, clearUnreadDirect, clearUnreadGroup, markGroupUnread, setActiveChat, refreshChatGroups } = useChatNotifications(session?.user?.id, addNotification, notifOn);
  useEventNotifications(notifOn, session?.user?.id, t);
  useRealtimeNotifications(session?.user?.id, addNotification, notifOn, t, markGroupUnread);
              ...
              <div style={{
                ...
                {isAdmin && (
                  <button onClick={() => { closeMenu(); navigateTo("admin"); play("F5", "16n"); }}
                    className="home-tile"
                    style={{ height: 64, flexShrink: 0, background: "#F5C800", border: "none", padding: "0 24px", cursor: "pointer", color: "#1a1a1a", textAlign: "left", display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div className="home-enter" style={{ margin: -28, height: "100vh", overflow: "hidden",
            ...
            >
              ...
              <div style={{
                ...
                <button onClick={() => { setDesktopSettingsOpen(true); play("F5", "16n"); }} aria-label="Settings" style={{
                  width: 44, height: 44,
                  display: "flex", alignItems: "center", justifyContent: "center",
                  background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.25)",
                  color: "#fff", cursor: "pointer",
            ...
            {/* Bottom bar — About / Profile / Admin */}
            <div style={{ gridRow: "2", gridColumn: "1", display: "grid", gap: 4, background: "#000", gridTemplateColumns: isAdmin ? "1fr 1fr 100px" : "1fr 1fr", minHeight: 0 }}>
              ...
              {isAdmin && (
                <button onClick={() => { navigateTo("admin"); play("F5", "16n"); }} onMouseEnter={() => play("D5", "64n")}
                  className="home-tile"
                  style={{ background: "#F5C800", border: "none", padding: "16px 16px", cursor: "pointer", color: "#1a1a1a", textAlign: "left", display: "flex", alignItems: "center", gap: 10 }}>
      ...
      {view === "grid" && currentMandalartId && (
        <div className="fade-in">
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 18, flexWrap: "wrap", gap: 10 }}>
            ...
            ) : <TopControls pal={pal} dark={dark} setDark={setDark} lang={lang} setLang={setLang} theme={theme} setTheme={setTheme} soundOn={soundOn} setSoundOn={setSoundOn} onBellClick={() => setNotifPanelOpen(true)} unreadCount={unreadCount} onSettingsClick={() => { setDesktopSettingsOpen(true); play("F5", "16n"); }} t={t} play={play} music={music} dropdownUp={false} onHome={() => navigateTo("home")} />}
          </div>
        ...
        <div className="fade-in">
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1px 1fr", gap: isMobile ? "28px 0" : "0 28px", alignItems: "start" }}>
            ...
            <div style={{ display: isMobile && profileTab !== "social" ? "none" : undefined }}>
              <ChatPanel
                pal={pal} dark={dark} t={t} myId={myId} myUsername={profile?.username}
                addNotification={addNotification}
